...
6. `ffmpeg` muxes a soft ASS subtitle track into an MKV (video and audio are copied). Audio-only files get a black 1080p 24 fps video track so the subtitles have a picture to sit on. ASS lines are hard-wrapped with `\\N`.
